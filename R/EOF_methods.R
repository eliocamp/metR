...
screeplot.eof <- function(x, npcs = "all", type = NULL, main = NULL, ...) {
  ...
  assertClass(x, "eof", add = checks)
  assert(
    checkCharacter(npcs, fixed = "all", any.missing = FALSE, len = 1),
  ...
  r2 <- "r2"
  if (npcs[1] == "all") {
    npcs <- as.numeric(unique(x$sdev[[var]]))
  }
  ggplot2::ggplot(
    x$sdev[as.numeric(get(var)) %in% npcs],
    ggplot2::aes(.data[[var]], .data[["r2"]])
  ) +
    ggplot2::geom_point() +
...
predict.eof <- function(object, n = NULL, ...) {
  ...
  right.vars <- colnames(object$right)[
    !(colnames(object$right) %in% c(pc, value.var))
  ]
  right.formula <- as.formula(paste0(
    pc,
    " ~ ",
    paste0(right.vars, collapse = "+")
  ))

  right <- object$right[object$sdev, on = pc][,
    (value.var) := get(value.var) * sd
  ]
  right <- .tidy2matrix(right, right.formula, value.var)

  left.vars <- colnames(object$left)[
    !(colnames(object$left) %in% c(pc, value.var))
  ]
  left.formula <- as.formula(paste0(
    pc,
    " ~ ",
    paste0(left.vars, collapse = "+")
  ))
  left <- .tidy2matrix(object$left, left.formula, value.var)

  dt <- cbind(
    .extend.dt(left$coldims, each = nrow(right$coldims)),
...
summary.eof <- function(object, ...) {
  ...
  cat(
    gettext("Component"),
    gettext("Explained variance"),
    gettext("Cumulative variance\n")
  )
  p <- lapply(seq_len(nrow(sdev)), function(x) {
    cat(
      formatC(sdev[x, ]$PC, width = 7),
      "  ",
