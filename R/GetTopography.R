...
GetTopography <- function(
  ...
  if (lat.north < lat.south) {
    stopf("'lat.north' can't be smaller than 'lat.south'.")
  }
  if (lon.west > lon.east) {
    stopf("'lon.east' can't be smaller than '.lon.west'.")
  }
  ...
  if (lon.west < 180 & lon.east > 180) {
    ...
    if (cache == TRUE && file %in% files) {
      if (verbose == TRUE) {
        messagef("Fetching cached field.")
      }
      field <- data.table::fread(file)[, -1]
    } else {
      # Get data
      url <- .BuildETOPORequest(
        lon.west,
        lon.east,
        lat.north,
        lat.south,
