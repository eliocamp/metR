...
Impute2D <- function(formula, data = NULL, method = "interpolate") {
  ...
  for (var in dep.names) {
    ...
    data.table::set(
      ...
      NULL,
      var,
      .impute_data(data_sub, na.fill = method, verbose = FALSE)$z
    )
  }
...
.impute_data <- function(data, na.fill = TRUE, verbose = TRUE) {
  ...
  if (nas != 0) {
    if (isTRUE(na.fill)) {
      if (isTRUE(verbose)) {
        warningf("Imputing missing values.", call. = FALSE)
      }
      data <- data.table::copy(data)[, z := soft_approx2d(x, y, z)]
      if (sum(is.na(data[["z"]])) != 0) {
        warningf(
          "Linear imputation failed. Try passing a constant number or a function (e.g. 'na.fill = mean')."
        )
      }
