...
Trajectory <- function(
  ...
  field <- data.table::as.data.table(eval(quote(model.frame(
    formula,
    data = data,
    na.action = NULL
  ))))
  ...
  points_out <- list(data.table::data.table(
    x = x0,
    y = y0,
    id = seq_along(x0),
    piece = 1,
  ...
  if (.is.somedate(field$t)) {
    points_out[,
      t1 := start <- lubridate::ymd_hms("1970-01-01 00:00:00") +
...
force.fun3d <- function(x, y, ts, times, field) {
  ...
  if (isTRUE(ts %in% times)) {
    ...
    force <- force[, .(dx = mean(dx), dy = mean(dy)), by = .(x, y)]
    return(force[, .(dx, dy)])
  }
}
