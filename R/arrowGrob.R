...
makeContent.arrow2 <- function(x) {
  ...
  x$x <- fast.unit.c(x$x - dx * x$pivot, x$x + dx * (1 - x$pivot))

  x$y <- fast.unit.c(x$y - dy * x$pivot, x$y + dy * (1 - x$pivot))
...
makeContent.vector <- function(x) {
  ...
  x$x <- grid::unit.c(x$x - x$dx * x$pivot, x$x + x$dx * (1 - x$pivot))

  x$y <- grid::unit.c(x$y - x$dy * x$pivot, x$y + x$dy * (1 - x$pivot))
