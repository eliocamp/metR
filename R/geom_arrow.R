...
draw_key_vector <- function(data, params, size) {
  ...
  grob <- grid::segmentsGrob(
    x0 = 0.1,
    y0 = 0.5,
    x1 = grid::unit(0.1, "npc") + grid::unit(data$mag, "cm"),
    y1 = 0.5,
    gp = grid::gpar(
...
StatArrow <- ggplot2::ggproto(
  ...
  setup_data = function(data, params) {
    ...
    if (is.null(data$mag) | is.null(data$angle)) {
      if (is.null(data$dx) | is.null(data$dy)) {
        stopf(
          "stat_arrow needs wither 'mag' and 'angle' or 'dx' and 'dy'.",
          call. = FALSE
        )
      }
    ...
    data <- subset(
      data,
      x %in%
        JumpBy(sort(unique(x)), params$skip.x + 1) &
  ...
  compute_panel = function(
    ...
    if (preserve.dir == FALSE) {
      ...
      if (trans_name == "date") {
        warningf(
          "The use of preserve.dir = FALSE with date scales is not recommended.",
          call. = FALSE
        )
      }
    }
