...
#' @format NULL
#' @export
GeomContourTanaka <- ggplot2::ggproto(
  "GeomContourTanaka",
  ggplot2::GeomPath,
  draw_panel = function(
    data,
    ...
    munched[,
      relative.angle := c(relative.angle[-.N], relative.angle[.N]),
      by = group
    ]
    ...
    if (smooth > 0) {
      ...
      munched[,
        shade := as.vector(stats::filter(shade, w, circular = TRUE)),
        by = group
      ]
    }
    ...
    attr <- plyr::ddply(munched, "group", function(df) {
      ...
      data.frame(
        solid = identical(as.integer(linetype), 1L) ||
          identical(linetype, "solid"),
    ...
    if (!solid_lines && !constant) {
      stopf(
        "%s: If you are using dotted or dashed lines, colour, size and linetype must be constant over the line.",
        "geom_path",
        call. = FALSE
      )
    }
...
.addpiece <- function(remove, piece) {
  cuts <- which(remove)
  if (length(remove) > max(cuts)) {
    cuts <- c(cuts, length(remove) + 1)
  }
  if (min(cuts) > 1) {
    cuts <- c(1, cuts)
  }
