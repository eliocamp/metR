...
GeomShadow <- ggplot2::ggproto(
  ...
  draw_panel = function(
    ...
    if (!coord$is_linear()) {
      stopf(
        "Non lineal coordinates are not implemented in GeomShadow.",
        call. = FALSE
      )
    } else {
      coords <- data.table::as.data.table(coord$transform(data, panel_scales))
      coords <- coords[
        x %in% JumpBy(unique(x), skip + 1) & y %in% JumpBy(unique(y), skip + 1)
      ]
      ...
      if (raster == TRUE) {
        ...
        x_pos <- as.integer(
          (coords$x - min(coords$x)) / ggplot2::resolution(coords$x, FALSE)
        )
        y_pos <- as.integer(
          (coords$y - min(coords$y)) / ggplot2::resolution(coords$y, FALSE)
        )
...
.cast_shadow <- function(coords, skip) {
  ...
  m <- as.matrix(m[, -1])
  coords[,
    alpha := .shadow(x, y, m, xdim, ydim, sun.angle, sun.altitude),
    by = .(x, y)
  ]
  return(coords$alpha)
}
...
.shadow <- function(x, y, m, xdim, ydim, azimuth, altitude) {
  ...
  angle <- suppressWarnings(max(
    (h - h0) / sqrt((xs - x)^2 + (ys - y)^2) / 10,
    na.rm = TRUE
  ))
