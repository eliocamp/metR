...
#' @export
guide_merge.colorbar2 <- function(guide, new_guide) {
    guide
}

# this guide is not geom-based.
#' @export
guide_geom.colorbar2 <- function(guide, layers, default_mapping) {
    ...
    if (length(ggplot2:::compact(guide_layers)) == 0) guide <- NULL
