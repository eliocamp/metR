...
guide_train.colorstrip <- function(guide, scale, aesthetic = NULL) {
  ...
  ticks <- as.data.frame(stats::setNames(
...
guide_gengrob.colorstrip <- function(guide, theme) {
  ...
  switch(
    ...
    "horizontal" = {
      ...
      if (!label.position %in% c("top", "bottom")) {
        stopf("Label position \"%s\" is invalid.", label.position)
      }

      barwidth <- grid::convertWidth(
        guide$barwidth %||% (theme_width * 5),
        "mm"
      )
    ...
    "vertical" = {
      ...
      barheight <- grid::convertHeight(
        guide$barheight %||% (theme_height * 5),
        "mm"
      )
    }
  )
  ...
  barlength.c <- switch(
    guide$direction,
    "horizontal" = barwidth.c,
    "vertical" = barheight.c
  )
  nbreak <- nrow(guide$key)

  tic_pos.c <- scales::rescale(
    guide$key$.value,
    c(0.5, guide$nbin - 0.5),
    guide$bar$value[c(1, nrow(guide$bar))]
  ) *
    barlength.c /
    guide$nbin
  grob.bar <-
  ...
  vgap <- height_cm(
    theme$legend.spacing.y %||% (0.5 * grid::unit(title_height, "cm"))
  )
    ...
    if (!guide$label) {
      ...
      hjust <- x <- guide$label.hjust %||%
        ...
        if (any(is.expression(guide$key$.label))) {
          ...
          switch(guide$direction, horizontal = 0.5, vertical = 0)
        }
  ...
  grob.ticks <-
    if (!guide$ticks) {
      ...
      grid::segmentsGrob(
        ...
        y1 = y1,
        default.units = "mm",
        gp = grid::gpar(col = "white", lwd = 0.5, lineend = "butt")
      )
    }
    ...
    "horizontal" = {
      switch(
        ...
        "top" = {
          ...
          vps <- list(bar.row = 3, bar.col = 1, label.row = 1, label.col = 1)
        },
        "bottom" = {
          bl_widths <- barwidth.c
          bl_heights <- c(barheight.c, vgap, label_height.c)
          vps <- list(bar.row = 1, bar.col = 1, label.row = 3, label.col = 1)
        }
      )
    },
    "vertical" = {
      switch(
        ...
        "left" = {
          ...
          vps <- list(bar.row = 1, bar.col = 3, label.row = 1, label.col = 1)
        },
        "right" = {
          bl_widths <- c(barwidth.c, vgap, label_width.c)
          bl_heights <- barheight.c
          vps <- list(bar.row = 1, bar.col = 1, label.row = 1, label.col = 3)
        }
      )
    }
  )
