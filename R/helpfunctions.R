...
interleave.unit <- function(...) {
  do.call(
    "grid::unit.c",
    do.call("interleave.default", plyr::llply(list(...), as.list))
  )
}
#' @export
...
rename_aes <- function(x) {
  ...
  if (length(duplicated_names) > 0L) {
    ...
    warningf(
      "Duplicated aesthetics after name standardisation: %s",
      duplicated_message,
      call. = FALSE
    )
  }
...
downsample <- function(x, y, value, byx = 1, byy = 1, fill = mean) {
  ...
  data <- subset(
    data,
    x %in% JumpBy(sort(unique(x)), byx) & y %in% JumpBy(sort(unique(y)), byy)
  )
  data
}
