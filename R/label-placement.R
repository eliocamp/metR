...
label_placer_fraction <- function(
  frac = 0.5,
...
#' @keywords internal
#' @rdname label_placers
label_placement_fraction <- function(
  frac = 0.5,
  rot_adjuster = isoband::angle_halfcircle_bottom()
) {
...
label_placer_fraction_xy <- function(frac = 0.5) {
  ...
  function(x, y) {
    ...
    places <- vapply(
      frac,
      function(x) which.min(abs(d - max(d * x))),
      numeric(1)
    )
    return(places)
  }
...
#' @export
#' @rdname label_placers
label_placer_random <- function(
  seed = 42,
  n = 1,
  rot_adjuster = isoband::angle_halfcircle_bottom()
) {
  as_placer(
    label_placer_random_xy(seed = seed, n = n),
...
#' @keywords internal
#' @rdname label_placers
label_placement_random <- function(
  seed = 42,
  n = 1,
  rot_adjuster = isoband::angle_halfcircle_bottom()
) {
...
#' @export
#' @rdname label_placers
label_placer_flattest <- function(
  ref_angle = 0,
  rot_adjuster = isoband::angle_halfcircle_bottom()
) {
  as_placer(
    label_placer_flattest_xy(ref_angle = ref_angle),
    rot_adjuster = rot_adjuster
  )
}
...
#' @keywords internal
#' @rdname label_placers
label_placement_flattest <- function(
  ref_angle = 0,
  rot_adjuster = isoband::angle_halfcircle_bottom()
) {
...
label_placer_minmax <- function(
  ...
  as_placer(
    label_placer_minmax_xy(direction = direction),
    rot_adjuster = rot_adjuster
  )
}
...
#' @keywords internal
#' @rdname label_placers
label_placement_minmax <- function(
  direction = c("vertical", "horizontal"),
  rot_adjuster = isoband::angle_halfcircle_bottom()
) {
