...
#' @export
scale_fill_discretised <- function(
  ...,
  low = "#132B43",
  high = "#56B1F7",
  space = "Lab",
...
#' @export
scale_fill_divergent_discretised <- function(
  ...,
...
#' @usage NULL
#' @export
ScaleDiscretised <- ggplot2::ggproto(
  "ScaleDiscretised",
  ggplot2::ScaleBinned,
  transform = function(self, x) {
    ...
    if (is.numeric(self$limits)) {
      ...
      if (!identical(limits, self$limits)) {
        ...
        warningf(
          "User supplied limits don't correspond to valid breaks. %s rounded to %s",
          original_limits,
          new_limits
        )
      }
    ...
    self$breaks <- sort(unique(c(
      breaks[breaks >= self$limits[1] & breaks <= self$limits[2]],
      self$limits
    )))

    self$scale_limits <- c(
      diff(self$breaks)[1] / 2 + self$breaks[1],
  ...
  map = function(self, x, limits = self$get_limits()) {
    ...
    a <- ggplot2::ggproto_parent(ggplot2::ScaleContinuous, self)$map(
      new_x,
      self$scale_limits
    )
    a
...
get_middle <- function(x) {
  ...
  breaks <- levels(x)
  splitted <- strsplit(
    gsub("[\\(\\[\\)\\]]", "", as.character(breaks), perl = TRUE),
    ","
  )
