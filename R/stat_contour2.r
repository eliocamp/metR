...
StatContour2 <- ggplot2::ggproto(
  ...
  compute_group = function(
    ...
    if (kriging) {
      ...
      data <- try(
        with(
          data,
          setNames(
            kriging::kriging(x, y, z, pixels = pixels)$map,
            c("x", "y", "z")
          )
        ),
        silent = TRUE
    ...
    contours <- data.table::as.data.table(.contour_lines(
      data,
      breaks,
      complete = complete,
      clip = clip,
...
isolines_from_data.table <- function(x) {
  lapply(split(x, by = "level"), function(dt) {
    list(x = dt$x, y = dt$y, id = dt$id)
  })
}
...
remove_degenerates <- function(x) {
  ...
  degenerate_points <- x[, all(x == x[1]) & all(y == y[1]), by = .(id, level)][
    V1 == TRUE
  ]
...
.order_contour <- function(contours, data) {
  ...
  contours[,
    c("dx", "dy") := .(c(diff(x), NA), c(diff(y), NA)),
    by = .(level, id)
  ]
  ...
  segments[,
    c("x.axis", "y.axis") := .(x %in% x.data, y %in% y.data),
    by = .(level, id)
  ]
  ...
  rot.groups <- unique(rbind(
    y.axis[rotate == TRUE][, .(id, level)],
    x.axis[rotate == TRUE][, .(id, level)]
  ))
  # rot.groups <- c(as.character(y.axis$group), as.character(x.axis$group))

  rotated <- contours[rot.groups, on = c("id", "level")][,
    .SD[.N:1],
    by = .(id, level)
  ]
...
.contour_lines <- function(
  ...
  if (reorder) {
    cl <- isolines_from_data.table(.order_contour(
      isolines_as_data.table(cl),
      data
    ))
  }
    ...
    if (is.function(proj)) {
      ...
      if (is.character(proj)) {
        if (!requireNamespace("proj4", quietly = TRUE)) {
          stopf(
            "Projection requires the proj4 package. Install it with 'install.packages(\"proj4\")'."
          )
        }
        cl <- lapply(cl, function(x) {
          x[c("x", "y")] <- proj4::project(
            list(x$x, x$y),
            proj,
            inverse = proj.latlon
          )
  ...
  cont <- data.table::rbindlist(
    lapply(cl, data.table::as.data.table),
    idcol = "level"
  )
...
#' @format NULL
#' @export
StatTextContour <- ggplot2::ggproto(
  "StatTextContour",
  StatContour2,
  required_aes = c("x", "y", "z"),
  default_aes = ggplot2::aes(
    order = ggplot2::after_stat(level),
    label = ggplot2::after_stat(level)
  ),
  dropped_aes = "z"
)
