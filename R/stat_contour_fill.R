...
#' @format NULL
#' @export
StatContourFill <- ggplot2::ggproto(
  "StatContourFill",
  ggplot2::Stat,
  ...
  compute_group = function(
    ...
    if (!isFALSE(kriging)) {
      ...
      data <- try(
        with(
          data,
          setNames(
            kriging::kriging(x, y, z, pixels = kriging)$map,
            c("x", "y", "z")
          )
        ),
        silent = TRUE
    ...
    cont <- data.table::setDT(.contour_bands(
      data,
      breaks,
      complete = complete,
      clip = clip,
...
.contour_bands <- function(
  ...
  cont[, `:=`(
    level_low = as.numeric(level_low),
    level_high = as.numeric(level_high)
  )]
  ...
  cont[,
    group := factor(paste(data$group[1], sprintf("%03d", piece), sep = "-"))
  ]
