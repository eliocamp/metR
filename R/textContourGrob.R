...
makeContent.contourTextGrob <- function(x) {
  ...
  hj <- unlist(lapply(payload$data, function(x) {
    grid::resolveHJust(x$just, NULL)
  }))[valid]
  vj <- unlist(lapply(payload$data, function(x) {
    grid::resolveVJust(x$just, NULL)
  }))[valid]
...
labelGrob <- function(
  ...
  y = grid::unit(0.5, "npc"),
  just = "center",
  padding = grid::unit(0.25, "lines"),
  r = grid::unit(0.1, "snpc"),
  default.units = "npc",
  name = NULL,
  text.gp = grid::gpar(),
  rect.gp = grid::gpar(fill = "white"),
  vp = NULL
) {
  if (length(label) != 1) {
