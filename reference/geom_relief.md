...
plotting topographic data because relief shading might give a more
intuitive impression of the shape of the terrain than contour lines or
mapping height to colour. `geom_shadow()` projects shadows.
...
- sun.angle:
  ...
  angle from which the sun is shining, in degrees counterclockwise from
  12 o' clock

- raster:

  if `TRUE` (the default), uses
...
`light` and `dark` must be valid colours determining the light and dark
shading (defaults to "white" and "gray20", respectively).

## Aesthetics

`geom_relief()` and `geom_shadow()` understands the following aesthetics
