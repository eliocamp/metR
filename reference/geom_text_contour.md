...
Draws labels on contours built with
...
- stroke:
  ...
  numerical indicating width of stroke relative to the size of the text.
...
Is best used with a previous call to
[ggplot2::stat_contour](https://ggplot2.tidyverse.org/reference/geom_contour.html)
with the same parameters (e.g. the same `binwidth`, `breaks`, or
`bins`). Note that while `geom_text_contour()` can angle itself to
follow the contour, this is not the case with `geom_label_contour()`.

## Aesthetics

`geom_text_contour` understands the following aesthetics (required
