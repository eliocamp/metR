...
Removes values where `subset` evaluates to `FALSE`. Useful for showing
only statistical significant values, or an interesting subset of the
data without manually subsetting the data.
...
`stat_subset` understands the following aesthetics (required aesthetics
are in bold)
...
[stat_na](https://eliocamp.github.io/metR/reference/stat_na.md) for a
more specialized stat for filtering `NA` values.
