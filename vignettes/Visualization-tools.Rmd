...
The function `RepeatLon()` is an ugly kludge to fix an issue when using polar coordinates in ggplot2 in which contours that cross the dateline are not properly rendered. The workaround is to copy the leftmost data and place it to the right.
...
g %+% RepeatLon(nceptemperature[lev == 300])    # change the data. 
```
