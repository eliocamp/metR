...
# dimensions
file <- system.file("extdata", "nceptemperature.nc", package = "metR")
...
aao[, gh.t.w := Anomaly(gh)*sqrt(cos(lat*pi/180)), by = .(lon, lat, month(date))]

eof <- EOF(aao, date ~ lon + lat, value.var = "gh.t.w", n = 1:2)
str(eof)
```
...
```{r}
eof2 <- EOF(aao, lon + lat ~ date, value.var = "gh.t.w", n = 1:2)
str(eof2)
```
...
```{r}
aao[date == date[1],    # think: gh as a function of lon and lat
